...
def StackAllPhasesHists(NeuronPhases_Pyr, NeuronPhases_Int, bins_vect, groups_pyr=None, groups_int=None):
    
    if groups_pyr is None:
        groups_pyr=[range(4000)]
        
    NeuronPhasesHists_Pyr = {'ING': np.zeros([4000, len(bins_vect)-1]), 'PING': np.zeros([4000, len(bins_vect)-1])}
    ...
    if groups_int is None:
        groups_int=[range(1000)]
        
    NeuronPhasesHists_Int = {'ING': np.zeros([1000, len(bins_vect)-1]), 'PING': np.zeros([1000, len(bins_vect)-1])}
